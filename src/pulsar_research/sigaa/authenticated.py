...
from ..ingest.legacy import parse_slots, resolve_opportunity_professors
...
def _legacy_paths(config: AppConfig) -> tuple[Path, Path]:
    return config.paths.legacy_ledger_json, config.paths.legacy_ledger_csv
...
def merge_legacy_ledger(config: AppConfig, ledger: dict[str, dict[str, Any]]) -> None:
    path = config.paths.legacy_ledger_json
    if not path.exists():
        return
    try:
...
def save_legacy_ledger(config: AppConfig, ledger: dict[str, dict[str, Any]]) -> None:
    json_path, csv_path = _legacy_paths(config)
...
def _credentials() -> tuple[str, str]:
    return env_secret("UFAL_SIGAA_USERNAME"), env_secret("UFAL_SIGAA_PASSWORD")
...
def sync_opportunities(config: AppConfig) -> dict[str, Any]:
    ...
    ledger = load_ledger_from_db(db); merge_legacy_ledger(config, ledger)
    ...
    def save():
        ...
        save_legacy_ledger(config, ledger)
